...
# Install PyQt6 GUI v4.0 Modern UI Edition
echo "🎨 Installing v4.0 Modern UI Preferences GUI..."
cp "$SCRIPT_DIR/screensaver_preferences_v4.py" "$LOCAL_BIN/screensaver_preferences.py"
...
# Install logo files for v4 sidebar
echo "🖼️ Installing v4 logo files..."
if [ -f "$SCRIPT_DIR/sidekick_logo_dark.png" ]; then
    cp "$SCRIPT_DIR/sidekick_logo_dark.png" "$LOCAL_BIN/"
    echo "✅ Dark theme logo installed"
fi
if [ -f "$SCRIPT_DIR/sidekick_logo_light.png" ]; then
    cp "$SCRIPT_DIR/sidekick_logo_light.png" "$LOCAL_BIN/"
...
if [ -f "$SCRIPT_DIR/video_widget.py" ]; then
    echo "🎬 Installing Video Player Widget..."
    cp "$SCRIPT_DIR/video_widget.py" "$LOCAL_BIN/"
...
if [ -f "$SCRIPT_DIR/Matrix1.mp4" ] || [ -f "$SCRIPT_DIR/Matrix2.mp4" ]; then
    echo "🎥 Installing pre-rendered Matrix videos..."
    [ -f "$SCRIPT_DIR/Matrix1.mp4" ] && cp "$SCRIPT_DIR/Matrix1.mp4" "$VIDEOS_DIR/" && echo "✅ Matrix1.mp4 installed"
    [ -f "$SCRIPT_DIR/Matrix2.mp4" ] && cp "$SCRIPT_DIR/Matrix2.mp4" "$VIDEOS_DIR/" && echo "✅ Matrix2.mp4 installed"
    echo "   💡 These videos use ~2% CPU vs ~10% for live rendering!"
else
    echo "ℹ️  No pre-rendered Matrix videos found (Matrix1.mp4, Matrix2.mp4)"
...
if [ -d "$HOME/screensaver-media" ]; then
    ...
    # Copy videos if they exist in script dir
    [ -f "$SCRIPT_DIR/Matrix1.mp4" ] && cp "$SCRIPT_DIR/Matrix1.mp4" "$MEDIA_DIR/videos/" 2>/dev/null || true
    [ -f "$SCRIPT_DIR/Matrix2.mp4" ] && cp "$SCRIPT_DIR/Matrix2.mp4" "$MEDIA_DIR/videos/" 2>/dev/null || true
else
    mkdir -p "$MEDIA_DIR/videos"
    mkdir -p "$MEDIA_DIR/images"

    # Copy videos if available
    [ -f "$SCRIPT_DIR/Matrix1.mp4" ] && cp "$SCRIPT_DIR/Matrix1.mp4" "$MEDIA_DIR/videos/"
    [ -f "$SCRIPT_DIR/Matrix2.mp4" ] && cp "$SCRIPT_DIR/Matrix2.mp4" "$MEDIA_DIR/videos/"
...
if [ "$DISPLAY_AVAILABLE" = true ]; then
    ...
    if command -v zenity >/dev/null 2>&1; then
        # Use zenity for graphical prompt
