...
class SlideshowWidget(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        self.emergency_cpu_threshold = 90.0  # If system CPU > 90%, throttle slide transitions
        ...
        self.normal_slide_interval = int(self.settings.get('slide_duration', 5.0) * 1000)
    ...
    def update_fps(self):
        """Update FPS counter and check emergency CPU throttling"""
        ...
        # EMERGENCY CPU THROTTLING: Check system CPU
    ...
    def check_emergency_cpu_throttle(self):
        """Check system CPU and slow down slide transitions if over 90%"""
        try:
            import psutil
            ...
            if cpu_percent > self.emergency_cpu_threshold:
                if not self.is_emergency_throttled:
                    # Enable emergency throttling - slow down slide transitions to 30 seconds
                    self.is_emergency_throttled = True
                    self.slide_timer.setInterval(30000)  # 30 seconds between slides
                    print(f"🚨 EMERGENCY THROTTLE: System CPU at {cpu_percent:.1f}% - slowing slideshow to 30s/slide")
            else:
                if self.is_emergency_throttled:
                    # Restore normal slide timing
                    self.is_emergency_throttled = False
                    self.slide_timer.setInterval(self.normal_slide_interval)
                    print(f"✅ THROTTLE RELEASED: System CPU at {cpu_percent:.1f}% - restoring normal slide timing")
